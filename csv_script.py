...
datas = {}
podaci = []
header_style = xlwt.easyxf("pattern: fore_colour dark_blue, pattern solid;" "font: colour white;")
special_style = xlwt.easyxf("pattern: fore_colour blue, pattern solid;" "font: colour white;")
kategorije_style = xlwt.easyxf("pattern: fore_colour light-blue, pattern solid;" "font: colour white;")
razrada_style = xlwt.easyxf("pattern: fore_colour blue_gray, pattern solid;" "font: colour white;")
...
with open('/home/matija/Desktop/Tabela.csv', 'r') as myfile:
    ...
    for d in data:
        ...
        if d[1] in datas:
            ...
            if d[2] in datas[d[1]]:
                datas[d[1]][d[2]].append((d[3], d[4], d[6], d[7], d[9], d[10], d[11]))
            else:
                datas[d[1]][d[2]] = [(d[3], d[4], d[6], d[7], d[9], d[10], d[11])]
        else:
            datas[d[1]] = {d[2]: [(d[3], d[4], d[6], d[7], d[9], d[10], d[11])]}
...
for partner in datas:
    ...
    for t in range(0, 22, 1):
        sheet.write(1, t, style=header_style)

    for w in range(0, 22, 1):
    ...
    for group in grupe:
        ...
        suu9, suu10, suu11, suu12, suu13, suu14, suu15, suu16, suu17 = 0, 0, 0, 0, 0, 0, 0, 0, 0
        kuu9, kuu10, kuu11, kuu12, kuu13, kuu14, kuu15, kuu16, kuu17 = 0, 0, 0, 0, 0, 0, 0, 0, 0
        sumarum = row
        row += 1
        for g in group:
            ukupnog=[]
            for x in group[g]:
                su9, su10, su11, su12, su13, su14, su15, su16, su17 = 0, 0, 0, 0, 0, 0, 0, 0, 0
                ku9, ku10, ku11, ku12, ku13, ku14, ku15, ku16, ku17 = 0, 0, 0, 0, 0, 0, 0, 0, 0
                firstrowG = row
                sheet.write(row, 1, x, style=kategorije_style)
                for c in range(2, 22, 1):
                    sheet.write(row, c, style=kategorije_style)
                row += 1
                for d in group[g][x]:
                    #zapis sume po razradi
                    s9, s10, s11, s12, s13, s14, s15, s16, s17 = 0, 0, 0, 0, 0, 0, 0, 0, 0
                    k9, k10, k11, k12, k13, k14, k15, k16, k17 = 0, 0, 0, 0, 0, 0, 0, 0, 0
                    sheet.write(row, 2, d, style=razrada_style)
                    for c in range(3, 22, 1):
                    ...
                    for prod in produkt:
                        for godina in produkt[prod]:
                            ...
                            if tmp == prod:
                                ...
                                sheet.write(row, 3, prod)
                                row += 1
                            for let in produkt[prod][godina]:
                                #zbog zapisa podataka o godini jedan red prenisko
                                row = row - 1
                                if godina == '2009':
                                    sheet.write(row, 4, let[0])
                                    sheet.write(row, 5, let[1])
                                    s9 += float(let[0])
                                    k9 += float(let[1])
                                    sheet.write(firstrow, 4, s9, style=razrada_style)
                                    sheet.write(firstrow, 5, k9, style=razrada_style)
                                    su9 += float(let[0])
                                    ku9 += float(let[1])
                                    sheet.write(firstrowG, 4, su9, style=kategorije_style)
                                    sheet.write(firstrowG, 5, ku9, style=kategorije_style)
                                    suu9 += float(let[0])
                                    kuu9 += float(let[1])
                                    sheet.write(sumarum, 4, suu9, style=special_style)
                                    sheet.write(sumarum, 5, kuu9, style=special_style)
                                    tmp = prod
                                elif godina == '2010':
                                    sheet.write(row, 6, let[0])
                                    sheet.write(row, 7, let[1])
                                    s10 += float(let[0])
                                    k10 += float(let[1])
                                    sheet.write(firstrow, 6, s10, style=razrada_style)
                                    sheet.write(firstrow, 7, k10, style=razrada_style)
                                    su10 += float(let[0])
                                    ku10 += float(let[1])
                                    sheet.write(firstrowG, 6, su10, style=kategorije_style)
                                    sheet.write(firstrowG, 7, ku10, style=kategorije_style)
                                    suu10 += float(let[0])
                                    kuu10 += float(let[1])
                                    sheet.write(sumarum, 6, suu10, style=special_style)
                                    sheet.write(sumarum, 7, kuu10, style=special_style)
                                    tmp = prod
                                elif godina == '2011':
                                    sheet.write(row, 8, let[0])
                                    sheet.write(row, 9, let[1])
                                    s11 += float(let[0])
                                    k11 += float(let[1])
                                    sheet.write(firstrow, 8, s11, style=razrada_style)
                                    sheet.write(firstrow, 9, k11, style=razrada_style)
                                    su11 += float(let[0])
                                    ku11 += float(let[1])
                                    sheet.write(firstrowG, 8, su11, style=kategorije_style)
                                    sheet.write(firstrowG, 9, ku11, style=kategorije_style)
                                    suu11 += float(let[0])
                                    kuu11 += float(let[1])
                                    sheet.write(sumarum, 8, suu11, style=special_style)
                                    sheet.write(sumarum, 9, kuu11, style=special_style)
                                    tmp = prod
                                elif godina == '2012':
                                    sheet.write(row, 10, let[0])
                                    sheet.write(row, 11, let[1])
                                    s12 += float(let[0])
                                    k12 += float(let[1])
                                    sheet.write(firstrow, 10, s12, style=razrada_style)
                                    sheet.write(firstrow, 11, k12, style=razrada_style)
                                    su12 += float(let[0])
                                    ku12 += float(let[1])
                                    sheet.write(firstrowG, 10, su12, style=kategorije_style)
                                    sheet.write(firstrowG, 11, ku12, style=kategorije_style)
                                    suu12 += float(let[0])
                                    kuu12 += float(let[1])
                                    sheet.write(sumarum, 10, suu12, style=special_style)
                                    sheet.write(sumarum, 11, kuu12, style=special_style)
                                    tmp = prod
                                elif godina == '2013':
                                    sheet.write(row, 12, let[0])
                                    sheet.write(row, 13, let[1])
                                    s13 += float(let[0])
                                    k13 += float(let[1])
                                    sheet.write(firstrow, 12, s13, style=razrada_style)
                                    sheet.write(firstrow, 13, k13, style=razrada_style)
                                    su13 += float(let[0])
                                    ku13 += float(let[1])
                                    sheet.write(firstrowG, 12, su13, style=kategorije_style)
                                    sheet.write(firstrowG, 13, ku14, style=kategorije_style)
                                    suu13 += float(let[0])
                                    kuu13 += float(let[1])
                                    sheet.write(sumarum, 12, suu13, style=special_style)
                                    sheet.write(sumarum, 13, kuu14, style=special_style)
                                    tmp = prod
                                elif godina == '2014':
                                    sheet.write(row, 14, let[0])
                                    sheet.write(row, 15, let[1])
                                    s14 += float(let[0])
                                    k14 += float(let[1])
                                    ...
                                    sheet.write(row, 16, let[0])
                                    sheet.write(row, 17, let[1])
                                    s15 += float(let[0])
                                    k15 += float(let[1])
                                    ...
                                    sheet.write(row, 18, let[0])
                                    sheet.write(row, 19, let[1])
                                    s16 += float(let[0])
                                    k16 += float(let[1])
                                    ...
                                    sheet.write(row, 20, let[0])
                                    sheet.write(row, 21, let[1])
                                    s17 += float(let[0])
                                    k17 += float(let[1])
